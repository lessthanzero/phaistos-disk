...
```mermaid
graph TD
    subgraph EcoLandscape [Bronze Age Messara Ecosystem]
        Palace["Phaistos Palace Ridge<br/>(95m elevation, administrative core)"]
        Plain["Alluvial Plain Hinterland<br/>(Cereals, pulses, olive groves, viticulture)"]
        River["Geropotamos / Lithaios River<br/>(Riparian plane trees, reeds, sweet galingale)"]
        Harbor["Kommos Port & Libyan Sea<br/>(Migratory tuna, longships, coastal fisheries)"]
        Mountain["Asterousia & Ida Ranges<br/>(Sheep/goat transhumance, raptors, wildcats)"]
    end

    Palace --> Plain --> River --> Harbor
    Palace --> Mountain

    subgraph FilterEngine [Epistemic Prior Filter]
        Morph["Sign Morphology"] --> Sieve{"6-Part Ecological Sieve"}
        EcoLandscape --> Sieve
        Sieve -->|Falsification| Rej["Rejected Anachronism / Exotic Error"]
        Sieve -->|Bayesian Prior| Conf["Validated Ecological Candidate"]
    end
```
